		...
		function fn_changeMemberInfo(){
			...
			if($(".Btn_pwd").val()=="취소"){
				if($("#pwd_chk").val()==${member.pwd}){
					if($("#pwd_new1").val()==$("#pwd_new2").val() && $("#pwd_new1").val()!='' && $("#pwd_new2").val()!=''){
						$("#_pwd").val($("#pwd_new1").val())
					}else{
			...
			if($(".Btn_NickName").val()=="취소"){
				if($("#nickname_new").val()==''){
					alert("새로운 닉네임을 입력하셔야 합니다")
					return false;
				}else{
			
					$("#_nicknmae").val($("#nickname_new").val())
					}
				}
			/*이메일 변경  */
			if($(".Btn_Email").val()=="취소"){
				if($("#email_new").val()==''){
					elert("새로운 이메일을 입력하셔야 합니다")
					return false;
				}else{
					$("#_email").val($("#email_new").val())
				}
			}
			...
			if($(".Btn_Ph").val()=="취소"){
				if($("#callnum_new").val()==''){
					elert("새로운 전화번호를 입력하셔야 합니다")
					return false;
				}else{
					$("#_callnum").val($("#callnum_new").val())
				}
			}
...
<body>
	<form id="chgForm" action="${contextPath }/memberInfo/updateMemberInfo.do" method="post">
		<div id="changeInfo">
			<table id="infoTable">
				<tr>
					...
					<td class="infoDetail">
						...
						<input type="hidden" id="_id"name="id" value="${member.id }"/>
					</td>
				...
				<tr>
					...
					<td class="infoDetail pwdInfo">
						...
						<input type="hidden" id="_pwd" name="pwd" value="${member.pwd }"/>
				...
				<tr class="changePwd">
					<td>
						<input id="pwd_chk" class="infoNew" type="password" placeholder="현재 비밀번호"/>
						<p class="infoConditon">현재 비밀번호를 입력해 주세요</p>
						<input id="pwd_new1" class="infoNew" type="password" placeholder="신규 비밀번호"/>
						<p class="infoConditon">8~20자 이내로 영문 대소문자,숫자를 모두 사용하여야 합니다</p>
						<input id="pwd_new2" class="infoNew" type="password"  placeholder="신규 비밀번호 확인"/>
				...
				<tr>
					...
					<td class="infoDetail pwdInfo">
						...
						<input type="hidden" id="_nicknmae" name="nickname" value="왜안대지" />
						<input class="Btn_change Btn_NickName" type="button" value="변경" onclick="fn_changeNickName()" />
					</td>
				</tr>
				<tr class="changeNickName">
					<td>
						<input id="nickname_new" class="infoNew" type="text" placeholder="신규닉네임"/>
				...
				<tr>
					...
					<td class="infoDetail emailInfo">
						...
						<input type="hidden" id="_email" name="email" value="${member.email }"/>
						<input class="Btn_change Btn_Email" type="button" value="변경" onclick="fn_changeEmail()" />
					</td>
				</tr>
				<tr class="changeEmail">
					<td>
						<input id="email_new" class="infoNew" type="text" placeholder="신규 이메일"/>
				...
				<tr>
					...
					<td class="infoDetail phInfo">
						...
						<input type="hidden" id="_callnum" name="callnum" value="${member.callnum }" />
						<input class="Btn_change Btn_Ph" type="button" value="변경" onclick="fn_changePh()" />
					</td>
				</tr>
				<tr class="changePh">
					<td>
						<input id="callnum_new" class="infoNew" type="text" placeholder="신규 전화번호"/>
